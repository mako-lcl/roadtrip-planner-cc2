...
fun createRoadtripPrompt(startLocation: String, endLocation: String, startDate: String, endDate: String, transportation: String) : String {
    return "You are a roadtrip planner. Format the response as a json. Plan a trip from " +
            ...
            " if it´s possible to get in the given time from $startDate to $endDate $transportation as is_possible, make sure that it is false if it isn´t possible and a list of locations with their name, latitude, longitude, the date you are there, " +
